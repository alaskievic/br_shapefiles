...
curso <- st_read(here::here("data", "rivers", "data", "curso_ana", "geoft_bho_cursodagua.shp"))
...
river <- st_read(here("data", "rivers", "data", "river_ana", "geoft_bho_rio.shp"))
  
# Create a dataset without geometry for visualization
river <- st_set_geometry(river, NULL)


### We are interested in cocursodag and nuareabacc to define the main rivers
...
#################### 2. Major Rivers for 1960-2010 AMCs ########################
...
curso_d4_names <- st_transform(curso_d4_names, crs = st_crs(amc_1960))
curso_d1000_names <- st_transform(curso_d1000_names, crs = st_crs(amc_1960))
curso_d5000_names <- st_transform(curso_d5000_names, crs = st_crs(amc_1960))


# Intersecting
river_d3 <- as_tibble(st_intersection(curso_d3_names,amc_1960))
river_d3 %<>% distinct(amc_1960_2) %>% mutate(river_d3 = 1)

river_d4 <- as_tibble(st_intersection(curso_d4_names, amc_1960))
river_d4 %<>% distinct(amc_1960_2) %>% mutate(river_d4 = 1)

river_d1000 <- as_tibble(st_intersection(curso_d1000_names, amc_1960))
river_d1000 %<>% distinct(amc_1960_2) %>% mutate(river_d1000 = 1)

river_d5000 <- as_tibble(st_intersection(curso_d5000_names, amc_1960))
river_d5000 %<>% distinct(amc_1960_2) %>% mutate(river_d5000 = 1)


# Merge back with shapefile
river_d3 <- full_join(amc_1960, river_d3, by = "amc_1960_2")
river_d4 <- full_join(amc_1960, river_d4, by = "amc_1960_2")
river_d1000 <- full_join(amc_1960, river_d1000, by = "amc_1960_2")
river_d5000 <- full_join(amc_1960, river_d5000, by = "amc_1960_2")
...
save(river_dummy_1960, file = here("output", "rivers", "river_dummy_1960amc.RData"))
